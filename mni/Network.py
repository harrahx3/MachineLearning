...
class Network :
    ...
    def train(self, inputs, targets, lr=0, activation=sigmoid, der_activation=d_sigmoid) :
        if lr == 0: lr=self.lr
        input = mat.Matrix.vector(inputs)
        ...
        self.weights_ih += error_h * hidden.compose(der_activation) * input.transpose() * lr
        
        self.bias_o += error_o * output.compose(der_activation) * lr
        self.bias_h += error_h * hidden.compose(der_activation) * lr